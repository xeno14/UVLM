...
std::vector<Eigen::Vector3d> wing_pos_initial;
...
void InitParam() {
  ...
  double h = H;
  double o = OMEGA;
  m.set_plug([h, o](double t) { return h * cos(o * t); });
}
...
auto CalcRhs(double t) {
  ...
  for (std::size_t K = 0; K < ROWS * COLS; ++K) {
    Eigen::Vector3d u =
        U + WakeVelocity(cpos[K]) - MorphingVelocity(wing_pos_initial[K], t);
  ...
  for (std::size_t i = 0; i < ROWS; i++) {
    for (std::size_t j = 0; j < COLS; j++) {
      ...
      std::vector<Eigen::Vector3d> corner_init = {
          get_pos(wing_pos_initial, i, j), get_pos(wing_pos_initial, i, j + 1),
          get_pos(wing_pos_initial, i + 1, j + 1),
          get_pos(wing_pos_initial, i + 1, j)};
...
void MainLoop(std::size_t step) {
  ...
  // TODO morphing
  CHECK(wing_pos.size() == wing_pos_initial.size());
  for (std::size_t i = 0; i < wing_pos.size(); i++) {
    m.Perfome(&wing_pos[i], wing_pos_initial[i], t);
  }
...
void SimulatorBody() {
  InitPosition(wing_pos_initial);
  wing_pos = wing_pos_initial;
  DT = 2 * M_PI / OMEGA / 40;
  // DT = 1./16.;
